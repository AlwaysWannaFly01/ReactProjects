...
// 获取区域租售比列表数据
export function getAreaRentalList(params) {
  return makeRequest({
    url: '/fdc/cityRentPriceRate/list/priceRentRate',
    method: 'GET',
    data: params
  })
}
...
// 区域租售比-导出
export function exportAreaRental(params) {
  const data = qs.stringify(params, { encodeValuesOnly: true })
  return makeRequest({
    url: `/fdc/excel/cityRentPriceRate/export/async?${data}`
  })
}
