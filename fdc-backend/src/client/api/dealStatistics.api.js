...
// 获取成交列表数据
export function getDealStatisticsList(params) {
  return makeRequest({
    url: '/fdc/projectDealCount',
    method: 'GET',
    data: params
  })
}
...
// 成交数据-导出
export function exportDealStatistics(params) {
  const data = qs.stringify(params, { encodeValuesOnly: true })
  return makeRequest({
    url: `/fdc/projectDealCount/export/async?${data}`
  })
}
