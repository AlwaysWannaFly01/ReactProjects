...
import * as serverApi from 'client/api/dealStatistics.api'
import * as dictApi from 'client/api/dict.api'
import actions from './actions'

// 获取成交列表数据
function* getDealStatisticsList() {
  while (true) {
    const {
      payload: [params, cb]
    } = yield take(actions.GET_DEAL_STATISTICS_LIST)
    try {
      const { data, code, message } = yield call(
        serverApi.getDealStatisticsList,
        params,
        actions.GET_DEAL_STATISTICS_LIST
      )
      yield put(actions.setDealStatisticsList(data))
...
// 区域租售比-导出
function* exportDealStatistics() {
  while (true) {
    const {
      payload: [params, cb]
    } = yield take(actions.EXPORT_DEAL_STATISTICS)
    try {
      yield call(serverApi.exportDealStatistics, params)
      cb()
    } catch (err) {
      handleErr(err)
    }
...
injectSaga({
  ...
  sagas: function* dataAreaRentalSaga() {
    yield fork(getDealStatisticsList)
    ...
    yield fork(exportDealStatistics)
