...
import * as serverApi from 'client/api/areaRental.api'
import * as dictApi from 'client/api/dict.api'
import actions from './actions'

// 获取区域租售比列表数据
function* getAreaRentalList() {
  while (true) {
    const {
      payload: [params, cb]
    } = yield take(actions.GET_AREA_RENTAL_LIST)
    try {
      const { data, code, message } = yield call(
        serverApi.getAreaRentalList,
        params,
        actions.GET_AREA_RENTAL_LIST
      )
      yield put(actions.setAreaRentalList(data))
...
// 区域租售比-导出
function* exportAreaRental() {
  while (true) {
    const {
      payload: [params, cb]
    } = yield take(actions.EXPORT_AREA_RENTAL)
    try {
      yield call(serverApi.exportAreaRental, params)
      cb()
    } catch (err) {
      handleErr(err)
    }
