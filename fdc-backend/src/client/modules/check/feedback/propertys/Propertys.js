...
/**
 * @description 数据审核-楼盘模块
 * @author ZLJ
 */

class Propertys extends Component {
  static propTypes = {
    form: PropTypes.object.isRequired,
    location: PropTypes.object.isRequired,
    model: PropTypes.instanceOf(Immutable.Map).isRequired,
    getPropertysList: PropTypes.func.isRequired,
    propertysList: PropTypes.array.isRequired,
  ...
  componentDidMount() {
    ...
    this.props.getPropertysList({ pageNum: 1, pageSize: 20 })
  }
  // 来源产品 (全选)
  ...
  handleSearch = (e, pageNum) => {
    ...
    this.props.getPropertysList(params)
  }
  // 城市范围变更事件
  ...
  renderTable() {
    ...
    const columns = [
      ...tableColumns,
      {
        title: '操作',
        width: '80',
        render: ({ id, dealingState }) => (
          <Fragment>
            {pagePermission('fdc:dc:feedbackCenter:sale:deal') ? (
              <Link
                to={{
                  pathname: router.FEEDBACK_PROPERTYS_REPLY,
                  search: `?id=${id}&state=${dealingState}`
                }}
              >
    ...
    return (
      <Table
        columns={columns}
        rowKey="key"
        dataSource={this.props.propertysList}
