 ...
 * @description 导入任务模块
 */
class ExportTaskds extends Component {
  static propTypes = {
  ...
  constructor(props) {
    ...
    const { type = '2' } = parse(props.location.search.substr(1))
    ...
    return (
      <Form onSubmit={e => this.handleSearch(e)} layout="horizontal">
        <Row>
          ...
          {pagePermission('fdc:ds:export:delete') ? (
            <Button
              type="danger"
              icon="delete"
              style={{ marginLeft: 16 }}
      ...
      },
      {
        ...
        render: ({ taskStatus, id }) =>
          ({
            1: (
              <Fragment>
                {pagePermission('fdc:ds:export:download') ? (
