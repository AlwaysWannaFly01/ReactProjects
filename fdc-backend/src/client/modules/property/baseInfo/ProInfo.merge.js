  ...
  handleMergeProject = () => {
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        const {
          projectItem: { areaId, projectId }
        } = values
        delete values.projectItem
        values.cityId = this.cityId
        values.srcAreaId = areaId
        values.srcPrjId = projectId
  
        values.projectName = this.props.projectDetail.get('projectName')
        values.targetAreaId = this.props.projectDetail.get('areaId')
        values.targetPrjId = this.props.projectDetail.get('id')
        values.areaName = this.props.projectDetail.get('areaName')
        values.currentProject = this.projectSelectRef.props.value.projectName
        values.currentAreaName = this.projectSelectRef.props.value.areaName
        // console.log(values, 1212)
        this.trans(values)
        return
