 ...
 */
/* eslint-disable */
class CaseInfoSample extends Component {
  static propTypes = {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (caseDate) {
          ...
          sessionStorage.setItem('CASE_INFO_SAMPLE_SEARCH', caseInfoSearch)
        }
      }
    })
  }
    ...
    return (
      <Form onSubmit={e => this.handleSearch(e, 1)} layout="horizontal">
        ...
        <Row style={{ marginBottom: 16 }}>
          <Link
            to={{
              pathname: router.RES_SAMPLE_CASEINFO_EDIT,
          ...
          {pagePermission('fdc:hd:residence:sampleCase:delete') ? (
            <Button
              type="danger"
              icon="delete"
          ...
          {pagePermission('fdc:hd:residence:sampleCase:allDelete') ? (
            <Popconfirm
          ...
          <Link
            to={{
              pathname: router.RES_SAMPLE_CASEINFO_IMPORT,
              search: `importType=${1212112}&cityId=${this.cityId}&cityName=${
                this.cityName
              }`
            }}
          >
            {pagePermission('fdc:hd:residence:sampleCase:import') ? (
          ...
          {pagePermission('fdc:hd:residence:sampleCase:export') ? (
