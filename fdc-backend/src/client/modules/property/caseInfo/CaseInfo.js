...
import CaseRangePicker3 from './CaseRangePicker3'
...
class CaseInfo extends Component {
  ...
  constructor(props) {
    ...
    const caseHappenDateEnd = caseHappenDateEndS
      ...
      : moment()
    // 导入日期
    const createTimeStart = createTimeStartS ? moment(createTimeStartS) : null
    const createTimeEnd = createTimeEndS ? moment(createTimeEndS) : null
  ...
  componentDidMount() {
    ...
    createTimeStart = createTimeStart
      ? createTimeStart.format('YYYY-MM-DD')
      : null
    createTimeEnd = createTimeEnd ? createTimeEnd.format('YYYY-MM-DD') : null
    ...
    this.cityIdInterval = setInterval(() => {
      ...
      if (this.cityId) {
        ...
        const qry = {
          ...
          checkedCaseTypeList: checkedCaseTypeList.join(','),
          caseHappenDateStart,
          caseHappenDateEnd,
          keyword,
          cityId: this.cityId,
        ...
        if (createTimeStart && createTimeEnd) {
          qry.createTimeStart = createTimeStart
          qry.createTimeEnd = createTimeEnd
        }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      this.setState(
        ...
        () => {
          // 清除查询条件
          sessionStorage.removeItem('CASE_INFO_SEARCH')
          // 初始化案例日期
          this.caseDate.initialDate(caseHappenDateStart, caseHappenDateEnd)
          // 初始化导入日期
          this.exportDate.initialDate()
          // 查询列表
          const qry = {
            caseHappenDateStart: caseHappenDateStart.format('YYYY-MM-DD'),
            caseHappenDateEnd: caseHappenDateEnd.format('YYYY-MM-DD'),
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (caseDate) {
          ...
          if (exportDate) {
            formData.createTimeStart = exportDate.startDate.format('YYYY-MM-DD')
            formData.createTimeEnd = exportDate.endDate.format('YYYY-MM-DD')
          }
          // console.log(formData)
          ...
          let createTimeEnd = ''
          if (exportDate) {
            createTimeStart = exportDate.startDate.format('YYYY-MM-DD')
            createTimeEnd = exportDate.endDate.format('YYYY-MM-DD')
          }
          let caseInfoSearch = `areaIds=${areaIds}&caseTypeCodes=${caseTypeCodes}&caseHappenDateStart=${caseHappenDateStart}&caseHappenDateEnd=${caseHappenDateEnd}&keyword=${keywordS ||
            ''}&pageNum=${pageNumS}&createTimeStart=${createTimeStart}&createTimeEnd=${createTimeEnd}`
          this.context.router.history.push({
            pathname: this.props.location.pathname,
            search: caseInfoSearch
          })
          sessionStorage.setItem('CASE_INFO_SEARCH', caseInfoSearch)
        }
      }
    })
  }
    ...
    return (
      <Form onSubmit={e => this.handleSearch(e, 1)} layout="horizontal">
          ...
          >
            {getFieldDecorator('exportDate')(
              <CaseRangePicker3
                ...
                onExportDateRef={this.onExportDateRef}
              />
            )}
          </FormItem>
        </Row>
        ...
        <Row style={{ marginBottom: 16 }}>
          <Link
            to={{
              pathname: router.RES_CASEINFO_EDIT,
          ...
          {pagePermission('fdc:hd:residence:case:delete') ? (
            <Button
              type="danger"
              icon="delete"
          ...
          {pagePermission('fdc:hd:residence:case:allDelete') ? (
            <Popconfirm
          ...
          <Link
            to={{
              pathname: router.RES_CASEINFO_IMPORT,
              search: `importType=${1212003}&cityId=${this.cityId}&cityName=${
                this.cityName
              }`
            }}
          >
            {pagePermission('fdc:hd:residence:case:import') ? (
          ...
          {pagePermission('fdc:hd:residence:case:export') ? (
