...
class CaseInfoEdit extends Component {
    ...
    this.cityIdInterval = setInterval(() => {
      ...
      if (this.cityId) {
        ...
        if (this.state.caseId) {
          this.props.getCaseDetail(this.state.caseId, this.cityId)
        }
      }
    }, 100)
    if (cityId) {
  ...
  handleDiffStandardPrice = (rule, value, callback) => {
    if (value !== undefined && value !== null && value <= 0) {
      callback('反调差标准价应大于0')
    }
    callback()
  ...
  handleDiffAvgPrice = (rule, value, callback) => {
    if (value !== undefined && value !== null && value <= 0) {
      callback('反调差均价应大于0')
    }
    callback()
  ...
  handleListingTotalPrice = (rule, value, callback) => {
    if (value !== undefined && value !== null && value <= 0) {
      callback('挂牌总价应大于0')
    }
    callback()
  ...
  handledealPeriod = (rule, value, callback) => {
    if (value !== undefined && value !== null && value <= 0) {
      callback('成交周期(天)应大于0')
    }
    callback()
  }
  
  handleWatchingCount = (rule, value, callback) => {
    if (value !== undefined && value !== null && value <= 0) {
      callback('带看次数应大于0')
    }
    callback()
  ...
  handleBalconyNum = (rule, value, callback) => {
    if (value !== undefined && value !== null && value <= 0) {
      callback('阳台数应大于0')
    }
    callback()
  ...
  handleWashroomNum = (rule, value, callback) => {
    if (value !== undefined && value !== null && value <= 0) {
      callback('洗手间数应大于0')
    }
    callback()
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        values.caseHappenDate =
          values.caseHappenDate &&
          values.caseHappenDate.format('YYYY-MM-DD HH:mm:ss')
        ...
        if (caseId) {
          ...
          this.props.editCase(values, res => {
            ...
            if (+code === 200) {
              ...
              // 列表查询条件
              const caseInfoSearch = sessionStorage.getItem('CASE_INFO_SEARCH')
              this.props.history.push({
                pathname: router.RES_CASEINFO,
  ...
  renderForm() {
    ...
    var isWithKitchenList = this.state.isWithKitchenList;

    let caseHappenDate =  fromJS(caseHappenDate)
    ...
    return (
      <Form onSubmit={this.handleSubmit}>
          ...
          <Col span={10}>
            <FormItem {...formItemLayout} label="反调差标准价">
              {getFieldDecorator('inverseDiffStandardPrice', {
                rules: [
                  {
                    validator: this.handleDiffStandardPrice
                  }
                ],
                initialValue: caseId
          ...
          <Col span={8}>
            <FormItem {...formItemLayout} label="反调差均价">
              {getFieldDecorator('inverseDiffAvgPrice', {
                rules: [
                  {
                    validator: this.handleDiffAvgPrice
                  }
                ],
                initialValue: caseId
        ...
        <Row>
          <Col span={10}>
            <FormItem {...formItemLayout} label="挂牌总价">
              {getFieldDecorator('listingTotalPrice', {
                rules: [
                  {
                    validator: this.handleListingTotalPrice
                  }
                ],
                initialValue: caseId
                    ? caseDetail.get('listingTotalPrice')
                    : undefined
              })(
                  <InputNumber
                      max={1000000000}
          ...
          <Col span={10}>
            <FormItem {...formItemLayout} label="是否有厨房">
              {getFieldDecorator('isWithKitchen', {
                initialValue: caseId
                    ? this.formatString(caseDetail.get('isWithKitchen'))
                    : undefined
              })(
                  <Select
                      style={{ width: '100%' }}
          ...
          <Col span={8}>
            <FormItem {...formItemLayout} label="阳台数">
              {getFieldDecorator('balconyNum', {
                rules: [
                  {
                    validator: this.handleBalconyNum
                  }
                ],
                initialValue: caseId
                    ? caseDetail.get('balconyNum')
                    : undefined
              })(
                  <InputNumber
                      max={1000000000}
          ...
          <Col span={10}>
            <FormItem {...formItemLayout} label="洗手间数">
              {getFieldDecorator('washroomNum', {
                rules: [
                  {
                    validator: this.handleWashroomNum
                  }
                ],
                initialValue: caseId
                    ? caseDetail.get('washroomNum')
                    : undefined
              })(
                  <InputNumber
                      max={1000000000}
        ...
        <Row>
          ...
          <Col span={8}>
            <FormItem {...formItemLayout} label="成交周期(天)">
              {getFieldDecorator('dealPeriod', {
                rules: [
                  {
                    validator: this.handledealPeriod
                  }
                ],
                initialValue: caseId ? caseDetail.get('dealPeriod') : undefined
              })(
                  <InputNumber
                      max={1000000000000000}
                      min={0}
        ...
        <Row>
          <Col span={10}>
            <FormItem {...formItemLayout} label="带看次数">
              {getFieldDecorator('watchingCount', {
                rules: [
                  {
                    validator: this.handleWatchingCount
                  }
                ],
                initialValue: caseId ? caseDetail.get('watchingCount') : undefined
              })(
                  <InputNumber
                      max={1000000000000000}
                      min={0}
        ...
        <Row>
          <Col span={10}>
            <FormItem {...formItemLayout} label=" " colon={false}>
              {caseId ? (
                <Fragment>
                  {pagePermission('fdc:hd:residence:case:change') ? (
                    <Button
                      type="primary"
                      loading={loading}
                      htmlType="submit"
                ...
                <Fragment>
                  {pagePermission('fdc:hd:residence:case:add') ? (
                    <Button
                      type="primary"
                      loading={loading}
                      htmlType="submit"
