...
import { Form, Table, Row, Col, Select, Button, Message } from 'antd'
...
// 面积确认
class IsAreaConfirmed extends Component {
  static propTypes = {
  ...
  handleSetArea = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { isAreaConfirmed } = this.props.form.getFieldsValue()
      if (isAreaConfirmed) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'isAreaConfirmed',
          fieldValue: isAreaConfirmed,
          floorList: floorNo,
          unitNoList: houseCol
        }
