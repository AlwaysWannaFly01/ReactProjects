...
// 建筑面积
class NoiseCode extends Component {
  static propTypes = {
    ...
    getNoiseType: PropTypes.func.isRequired,
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, noiseCode] = record[title].split(',')
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { noiseCode } = this.props.form.getFieldsValue()
      if (noiseCode) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'noiseCode',
          fieldValue: noiseCode,
          floorList: floorNo,
          unitNoList: houseCol
        }
  ...
  render() {
    ...
    const { noiseTypeList } = this.props.model
