...
// 阳台数
class BalconyNum extends Component {
  static propTypes = {
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, balconyNum] = record[title].split(',')
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { balconyNum } = this.props.form.getFieldsValue()
      if (balconyNum) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'balconyNum',
          fieldValue: balconyNum,
          floorList: floorNo,
          unitNoList: houseCol
        }
