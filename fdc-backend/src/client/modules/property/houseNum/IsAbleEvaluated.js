...
// 是否可估
class IsAbleEvaluated extends Component {
  static propTypes = {
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, isAbleEvaluated] = record[title].split(',')
                ...
                return (
                  ...
                  >
                    {isAbleEvaluated ? (
                      <span>{isAbleEvaluated === '1' ? '是' : '否'}</span>
                    ) : (
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { isAbleEvaluated } = this.props.form.getFieldsValue()
      if (isAbleEvaluated) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'isAbleEvaluated',
          fieldValue: isAbleEvaluated,
          floorList: floorNo,
          unitNoList: houseCol
        }
