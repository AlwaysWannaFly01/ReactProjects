...
import { Form, Table, Message, Row, Col, Select, Button } from 'antd'
...
// 是否证载 wy change 附属房屋类型
class IsBear extends Component {
  static propTypes = {
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, isBear] = record[title].split(',')
                return (
                  <Link
                    to={{
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { isBear } = this.props.form.getFieldsValue()
      if (isBear) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
