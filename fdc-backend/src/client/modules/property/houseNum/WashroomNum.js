...
import { Form, Table, Row, Message, Col, Button, InputNumber } from 'antd'
...
// 洗手间数
class WashroomNum extends Component {
  static propTypes = {
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, washroomNum] = record[title].split(',')
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { washroomNum } = this.props.form.getFieldsValue()
      if (washroomNum) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'washroomNum',
          fieldValue: washroomNum,
          floorList: floorNo,
          unitNoList: houseCol
        }
