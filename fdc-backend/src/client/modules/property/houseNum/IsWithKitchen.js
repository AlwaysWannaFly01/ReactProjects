...
// 是否有厨房
class IsWithKitchen extends Component {
  static propTypes = {
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, isWithKitchen] = record[title].split(',')
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { isWithKitchen } = this.props.form.getFieldsValue()
      if (isWithKitchen) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'isWithKitchen',
          fieldValue: isWithKitchen,
          floorList: floorNo,
          unitNoList: houseCol
        }
