...
// 朝向
class OrientationCode extends Component {
  static propTypes = {
    ...
    getOrientType: PropTypes.func.isRequired,
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, orientCode] = record[title].split(',')
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { orientationCode } = this.props.form.getFieldsValue()
      if (orientationCode) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'orientationCode',
          fieldValue: orientationCode,
          floorList: floorNo,
          unitNoList: houseCol
        }
  ...
  render() {
    ...
    const { orientTypeList } = this.props.model
