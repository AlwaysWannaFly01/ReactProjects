...
// 附属房屋类型
class SubHouseType extends Component {
  static propTypes = {
    ...
    getSubHouseType: PropTypes.func.isRequired,
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, subHouseType] = record[title].split(',')
                return (
                  <Link
                    to={{
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { subHouseType } = this.props.form.getFieldsValue()
      if (subHouseType) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'subHouseType',
          fieldValue: subHouseType,
          floorList: floorNo,
          unitNoList: houseCol
        }
