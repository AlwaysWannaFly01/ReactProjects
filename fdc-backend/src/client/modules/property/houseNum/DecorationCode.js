...
// 装修
class DecorationCode extends Component {
  static propTypes = {
    ...
    getDecorateType: PropTypes.func.isRequired,
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, decorationCode] = record[title].split(',')
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { decorationCode } = this.props.form.getFieldsValue()
      if (decorationCode) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'decorationCode',
          fieldValue: decorationCode,
          floorList: floorNo,
          unitNoList: houseCol
        }
  ...
  render() {
    ...
    const { decorateTypeList } = this.props.model
