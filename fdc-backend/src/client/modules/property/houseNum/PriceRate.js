...
class PriceRate extends Component {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                ...
                const [houseId, price, vqPrice] = record[title].split(',')
                ...
                return (
                  ...
                  >
                    {isPrice ? (
                      <span>
                        {price}{' '}
                        <span>{vqPrice >= 0 ? `(${vqPrice})` : null}</span>
                      </span>
                    ) : (
  ...
  render() {
    ...
    const { houseNameTitle = [], dataList = [], btnLoading } = this.state
      ...
      <div>
        {this.props.buildStatus === 1 ? (
          <Row style={{ marginTop: 8, marginBottom: 8 }}>
            {pagePermission('fdc:hd:residence:base:roomNum:calculate') ? (
              <Button
                onClick={this.handleCalculateHousePrice}
                type="primary"
                loading={btnLoading}
              >
                计算价格系数
