...
// 户型
class HouseTypeCode extends Component {
  static propTypes = {
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, houseTypeCode] = record[title].split(',')
                return (
                  <Link
                    to={{
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { houseTypeCode } = this.props.form.getFieldsValue()
      if (houseTypeCode) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'houseTypeCode',
          fieldValue: houseTypeCode,
          floorList: floorNo,
          unitNoList: houseCol
        }
  ...
  render() {
    ...
    const { houseTypeList } = this.props.model
