...
import { Form, Table, Row, Message, Col, Button, Select } from 'antd'
...
// 房号用途
class UsageCode extends Component {
  static propTypes = {
    ...
    getHouseUsage: PropTypes.func.isRequired,
  ...
  componentDidMount() {
    ...
    this.getDataList()
    // this.props.getUsageType()
    this.props.getHouseUsage()
  }

  componentWillReceiveProps(nextProps) {
    // const { houseUsageList } = this.props.model

    if (!shallowEqual(this.props.dataList, nextProps.dataList)) {
      const { houseTitle, dataList } = nextProps
      const { houseUsageList } = nextProps.model
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, usageCode] = record[title].split(',')
                let usageName = ''
                if (usageCode) {
                  const usageArr = houseUsageList.filter(
                    item => item.get('code') === usageCode
                  )
                  if (usageArr.size) {
                    usageName = usageArr.get(0).get('name')
                  }
                }
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { usageCode } = this.props.form.getFieldsValue()
      if (usageCode) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'usageCode',
          fieldValue: usageCode,
          floorList: floorNo,
          unitNoList: houseCol
        }
