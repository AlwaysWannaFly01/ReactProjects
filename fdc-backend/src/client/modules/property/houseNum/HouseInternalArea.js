...
import { Form, Table, Row, Message, Col, InputNumber, Button } from 'antd'
...
// 套内面积
class HouseInternalArea extends Component {
  static propTypes = {
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, houseArea] = record[title].split(',')
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { houseInternalArea } = this.props.form.getFieldsValue()
      if (houseInternalArea) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'houseInternalArea',
          fieldValue: houseInternalArea,
          floorList: floorNo,
          unitNoList: houseCol
        }
