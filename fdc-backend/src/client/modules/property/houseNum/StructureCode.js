...
// 户型结构
class StructureCode extends Component {
  static propTypes = {
    ...
    getStructType: PropTypes.func.isRequired,
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, structureCode] = record[title].split(',')
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { structureCode } = this.props.form.getFieldsValue()
      if (structureCode) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'structureCode',
          fieldValue: structureCode,
          floorList: floorNo,
          unitNoList: houseCol
        }
  ...
  render() {
    ...
    const { structTypeList } = this.props.model
