...
import { Form, Table, Row, Col, Button, Message, InputNumber } from 'antd'
...
// 价格系数
class RentPriceRate extends Component {
  static propTypes = {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                ...
                const [houseId, rentRate] = record[title].split(',')
  ...
  handleSetPrice = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { rentRate } = this.props.form.getFieldsValue()
      if (rentRate) {
        const data = {
          cityId: this.cityId,
