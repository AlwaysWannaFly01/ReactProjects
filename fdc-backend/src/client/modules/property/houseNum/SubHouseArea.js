...
import { Form, Table, Row, InputNumber, Col, Message, Button } from 'antd'
...
// 附属房屋面积
class SubHouseArea extends Component {
  static propTypes = {
        ...
        if (itemArr.length === 2) {
          ...
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, subHouseArea] = record[title].split(',')
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { subHouseArea } = this.props.form.getFieldsValue()
      if (subHouseArea) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
          fieldName: 'subHouseArea',
          fieldValue: subHouseArea,
          floorList: floorNo,
          unitNoList: houseCol
        }
