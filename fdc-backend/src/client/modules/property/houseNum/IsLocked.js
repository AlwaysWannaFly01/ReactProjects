...
// 是否锁定
class IsLocked extends Component {
  static propTypes = {
        ...
        if (itemArr.length === 2) {
          houseNameTitle.push({
            ...
            render: record => {
              ...
              if (record[title]) {
                const [houseId, isLocked] = record[title].split(',')
  ...
  handleSet = () => {
    ...
    if (isChosedHouseFloor) {
      ...
      const { isLocked } = this.props.form.getFieldsValue()
      if (isLocked) {
        const data = {
          cityId: this.cityId,
          buildingId: this.props.buildId,
